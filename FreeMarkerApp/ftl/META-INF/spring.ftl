 ...
 * @since 1.1 
 * modified by kernoli 2025/01/31
 -->
...
<#-- add by Kernoli -->
<#macro formDateTimePicker path attributes="" fieldType="text" format=""> 
    <@bind path/> 
    <input type="datetime-local" id="${status.expression}" name="${status.expression}" value="<#if fieldType!="password"><#if stringStatusValue?has_content ><#assign datetime=stringStatusValue?datetime("yyyy-MM-dd HH:mm")><#else><#assign datetime=.now></#if><#if format?exists && format?has_content>${datetime?string(format)}<#else>${datetime}</#if></#if>" ${attributes}<@closeTag/>
</#macro>
...
<#-- add by Kernoli -->
<#macro formComboBox path options attributes="" fieldType="text" defaultValue="">
    <@bind path/>
    <input list="${status.expression}List" id="${status.expression}" name="${status.expression}" value="<#if fieldType!="password"><#if stringStatusValue?has_content >${stringStatusValue}<#else>${defaultValue}</#if></#if>" ${attributes}<@closeTag/>
    <datalist id="${status.expression}List"> 
        <#if options?is_hash> 
            <#list options?keys as value>
...
<#-- add by Kernoli -->
<#macro formCheckboxes2 path options separator attributes="" selectedIndices="" >
    <@bind path/>
    <#list options?keys as value>
    <#assign id="${status.expression}${value_index}">
    <#assign isSelected = contains(status.value?default([""]), value) || selectedIndices?split(",")?seq_contains(value_index?string)>
    <input type="checkbox" id="${id}" name="${status.expression}" value="${value?html}"<#if isSelected> checked="checked"</#if> ${attributes}<@closeTag/>
    <label for="${id}">${options[value]?html}</label>${separator}
    </#list>
    <input type="hidden" name="_${status.expression}" value="on"/>
