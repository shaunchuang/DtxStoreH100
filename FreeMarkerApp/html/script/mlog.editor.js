...
$.extend(mlog.editor,{
	...
	init : function(conf){
		...
		if(conf.model != 'all' && conf.model != 'simple' && conf.model != 'platform') conf.model = 'all';
...
$.extend(mlog.editor.KindEditor, {
	...
	init : function(conf){
		...
		if(conf.language === undefined) conf.language = "zh_TW";
		if(conf.model === undefined) conf.model = 'all';
        ...
        'anchor', 'link', 'unlink'
			...
			this[conf.id] = KindEditor.create('#' + conf.id, {
				...
				memberOnly : true,
				items: [
						'cut', 'copy', 'paste', 'wordpaste', '|', 'insertorderedlist', 
						'insertunorderedlist', '|', 'indent', 'outdent', '|',
						'undo', 'redo', '|', 'bold', 'italic', 'underline', 
						'strikethrough', '|', 'justifyleft', 'justifycenter', 
						'justifyright', 'fontname', 'fontsize', '|', 
						'forecolor', 'hilitecolor', '|', 'image'
                ],
                afterCreate: function () {
                    // TODO: chrome bug
                    //window.onhashchange = admin.setCurByHash;
...
$.extend(mlog.editor.TinyMCE, {
	...
	init : function(conf){
		...
		if(conf.language === undefined) conf.language = "zh-tw";
		if(conf.model === undefined) conf.model = 'all';
